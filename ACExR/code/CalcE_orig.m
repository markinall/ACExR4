...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Specify constraints on H and V
H1_max = Const.H1max;
H1_min = Const.H1min;
if LochData.Nsill > 0
    H3_max = Const.H3max;
end
disp(['H1 min and max: ',num2str([H1_min H1_max]),'  m'])
...
for day = 1:Ndays
    ...
    if rem(day,30) == 0; disp(['Day ',num2str(day)]); end
    Param.day = day;
    ...
    T0(1) = mean(Bdata.T_ext(1:iz1,day));
    T0(2) = mean(Bdata.T_ext(iz1:iz2,day));
    S0(1) = mean(Bdata.S_ext(1:iz1,day));
    S0(2) = mean(Bdata.S_ext(iz1:iz2,day));
    rho0 =  Const.rho0 - alpha * (T0 - 10) + beta * (S0 - 35);    
    if Param.Ntracer > 0
        C0(1) = mean(Bdata.C_ext(1:iz1,day,:),1);
        C0(2) = mean(Bdata.C_ext(iz1:iz2,day,:),1);
    end
    ...
    for istep = 1:Nsteps_per_day
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Get new value of M
        Param.M = Const.g * Const.beta * D.S(1);

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Determine whether stratification is stable or unstable. If
        % unstable, deep water renewal is assumed to occur.
        D.delta = 0;
        if D.S(3) < D.S(2); D.delta = 1; end
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Calculate entrainment velocity, we, due to wind stirring
        % see eqns (6), (8) and (9) from Stigebrandt (1985).
        ...
        % The factor hupp tries to ensure that D.H(1) does not exceed H1_max
        %hupp = max([1-D.H(1)/H1_max, 0]);
        hupp = (H1_max - D.H(1))/(H1_max - H1_min);
        hupp = min(1, max(hupp,0));
        % The factor hlow tries to ensure that D.H(1) is not less than H1_min
        hlow = (D.H(1) - H1_min) / (H1_max - H1_min);
        hlow = min(1, max(hlow, 0));
        
        if Param.SHFlux == 1; 
            Qshf = SurfaceHeatFlux(D.T(1)); 
        else
            Qshf = 0;
        end
        B = Const.g * (Const.alpha * Qshf / (D.rho(1) * Const.cp)) ...
        ...
        if w_e < 0
            ...
            % h = Param.N / (Param.M * E.Qf)
            % We assume that this occurs over an e-folding time scale of 3
            % hours, giving:
            H_MO = max([Param.N(day) * Param.Af / (Param.M * E.Qf(day)), H1_min]);
            %w_e = (max([H_MO H1_min]) - D.H(1)) / (Const.Tday / 8);
            
            % H relaxes towards Monin-Obukhov length
            %w_e = min([0.5 * (H_MO - D.H(1)) / Const.deltaT, 0]);
            w_e = (0.5 * (H_MO - D.H(1)) / Const.deltaT);
        else
            %w_e = w_e * hupp;
        end
        ...
        if LochData.Nsill > 0
            ...
            D.Kzb = 2 * Kz_back * D.A_ifc(2) / (D.H(2) + D.H(3));
            %D.Kzb = 2 * Kz_back * D.A_ifc(2) / sum(D.H);
            D.Kz23 = KZ + D.Kzb;
            % Entrainment
            %D.Qw23 = Entrain23;
        else
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Sensitivity test: set all fluxes except tidal prism to zero
        %E.Qi(day) = 0;
        %E.Qf(day) = 0;
        %D.Qe = 0;
        %D.Qh = 0;
        ...
        %D.Kz12 = 0;
        %D.Kz23 = 0;
        %if Param.t == Const.deltaT
        %    disp('*****************************************')
        %    disp('WARNING: ALL FLUXES EXCEPT Qt SET TO ZERO')
        %    disp('*****************************************')
        %end
        ...
        V_new(1) = D.V(1) + Const.deltaT * (D.Qh + D.Qw12);
        V_new(2) = D.V(2) - Const.deltaT * (D.Qh + D.Qw12 - D.Qw23);
        V_new(3) = D.V(3) - Const.deltaT *  D.Qw23;
        ...
        Tnew = D.T;
        Snew = D.S;
        Tnew = Tracer(D.T,D.T(1),T0);
        %Tnew = TracerSI(D.T,D.T(1),T0);
        if Param.SHFlux == 1; 
            Qshf = SurfaceHeatFlux(D.T(1)); 
            Hbar = 0.5 * (D.Hp(1) + D.H(1));
            Tnew(1) = Tnew(1) + Const.deltaT * Qshf / ...
                (Const.cp * D.rho(1) * Hbar);
        else
            Qshf = 0;
        end
        [Snew] = Tracer(D.S,0,S0);
        %[Snew] = TracerSI(D.S,0,S0);
     
        % Tracers, if modelled
        if Param.Ntracer > 0
            for ic = 1:Param.Ntracer
                % Convert nutrient mass flux (kg/s) to concentration (kg/m3)
                Criver = LochData.RC(ic) / Bdata.Qf(day);
                Cnew(:,ic) = Tracer(D.C1,C1river,C10);
            end
        end
        ...
        Bdata.Qshf(day) = Bdata.Qshf(day) + Qshf;
    end
